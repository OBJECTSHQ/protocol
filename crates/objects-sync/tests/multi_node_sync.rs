...
async fn test_doc_sync_via_ticket() {
    ...
    let ticket = sync_a.docs().create_ticket(replica_id).await.unwrap();
    ...
    assert_eq!(entries.len(), 2, "Expected exactly 2 synced entries");
}
...
async fn test_project_sharing_via_ticket() {
    ...
    let p = retrieved_project.expect("Project should be retrievable on Node B after sync");
    ...
    let a = retrieved_asset.expect("Asset should be retrievable on Node B after sync");
